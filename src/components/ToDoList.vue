    ...
    <div class="toDoList_wrapper">
        <div class="toDoList_main">
            ...
            <div class="box" v-for="(item,index) in list" :key="item.id">
                ...
                <button @click="deleteData(index)" class="btn-xoa"><Icon icon="material-symbols:delete-outline"></Icon></button>
            </div>
        </div>
    </div>
...
function saveData(){
    const itemid = list.value.findIndex(item  => item.id === editedItemId.value)
    if(itemid !== -1){
        list.value[itemid].content=editedItemId.value
    }
}
